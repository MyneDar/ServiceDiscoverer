...
func (tx *Tx) init() {
	tx.EndpointData = NewEndpointDataClient(tx.config)
	tx.ProviderEndpoint = NewProviderEndpointClient(tx.config)
	tx.ProviderRegisterData = NewProviderRegisterDataClient(tx.config)
}
...
// applies a query, for example: EndpointData.QueryXXX(), the query will be executed
